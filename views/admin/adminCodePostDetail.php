    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="flex items-center justify-between p-4 bg-white border-b">
            <div class="flex items-center">
                <a href="adminCodePostView.php" class="text-gray-600 hover:bg-gray-100 p-2 rounded-full mr-2">
                    <span class="material-icons">arrow_back</span>
                </a>
                <h2 class="text-2xl font-semibold text-gray-800">Post Details</h2>
            </div>
            <div class="flex items-center space-x-4">
                <a href="adminCodePostEdit.php?id=<?= $post['coding_id'] ?>" class="bg-orange-600 text-white px-4 py-2 rounded-md hover:bg-orange-700 flex items-center">
                <span class="material-icons mr-2">edit</span> Edit Post
                </a>

                <a href="../../app/models/adminCodePostDelete.php?id=<?= $post['coding_id'] ?>" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 flex items-center"
                    ...
                    <div class="flex-1">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <span class="text-sm font-semibold inline-block py-1 px-3 uppercase rounded-full 
                                      <?= $post['level'] === 'Beginner' ? 'text-green-600 bg-green-200' : ($post['level'] === 'Intermediate' ? 'text-yellow-600 bg-yellow-200' : 'text-red-600 bg-red-200') ?>">
                    ...
                    <div class="w-full md:w-1/3">
                        ...
                        <?php if(!empty($post['coding_image'])): ?>
                            <img class="w-full h-64 object-cover rounded-lg shadow-sm" 
                                 src="data:image/jpeg;base64,<?= base64_encode($post['coding_image']); ?>" 
                                 alt="Class Image"/>
                        <?php else: ?>
