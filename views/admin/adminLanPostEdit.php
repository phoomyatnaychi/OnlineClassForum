...
<header class="flex items-center justify-between p-4 bg-white border-b">
    <div class="flex items-center">
        <button onclick="window.location.href='http://localhost/PHP_project/FourthYearDatabaseProject/OnlineClassForum/views/admin/adminLanPostView.php'" 
...
<form class="space-y-6" action="../../app/models/adminLanPostEdit.php" method="POST" enctype="multipart/form-data">
    <!-- hidden id so PHP knows which record to update -->
    <input type="hidden" name="language_id" value="<?php echo $post['language_id']; ?>">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            <label class="block text-sm font-medium text-gray-700" for="type">Language Type</label>
            <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm" 
                   id="type" 
                   name="type" 
                   type="text" 
        ...
        <div>
            ...
            <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm" 
                    ...
                    name="post-level"
                    type="text" 
                   value="<?php echo htmlspecialchars($post['level']); ?>"/>
    ...
    <?php if(!empty($post['lan_image'])): ?>
        <img id="preview" 
             src="data:image/jpeg;base64,<?php echo base64_encode($post['lan_image']); ?>" 
             alt="Class Image" 
    ...
    <input type="file" name="lan_image" id="lan_image"
           accept="image/*"
...
<!-- JS for live preview -->
<script>
document.getElementById("lan_image").addEventListener("change", function(event) {
